...
Le détail des choix d'architecture, la stratégie de branches et l'historique des incidents rencontrés pendant la mise en place de cette infrastructure sont couverts par la compétence RNCP C2.1.1 du rapport ; ce document décrit uniquement la procédure de déploiement.
...
- En production : un hôte (VPS ou équivalent) avec les ports 80 et 443 ouverts, un nom de domaine pointant dessus, et un certificat TLS déjà obtenu (Let's Encrypt) dans `/etc/letsencrypt` sur l'hôte, ce dossier étant monté en lecture seule dans le conteneur `nginx`
...
- **frontend** lancé avec `ng serve --host 0.0.0.0`, rechargement à chaud sur `http://localhost:4200`
...
**Alternative sans Docker** : `npm run install:all` puis `npm run dev` à la racine démarre en parallèle `symfony server:start` (port 8008), `ng serve` et le bot. Cela nécessite PHP 8.2+, Symfony CLI, Node 22 et une instance MySQL locale accessible en 3307.

### 3.1 Stack de supervision (optionnelle)

Décrite dans [SUPERVISION.md](SUPERVISION.md) ; commandes de démarrage et de vérification ici, puisqu'elles relèvent de l'exploitation plutôt que de la conception du système :

```bash
# Depuis la racine du projet : le .env résultant doit être à la racine (pas dans monitoring/),
# c'est là que Docker Compose le lit pour toutes les commandes -f, quel que soit leur nombre.
cp monitoring/.env.example .env   # puis renseigner DISCORD_WEBHOOK_URL et GRAFANA_ADMIN_PASSWORD dans ce .env
docker compose -f docker-compose.yml -f docker-compose.override.yaml -f docker-compose.monitoring.yaml up -d
```

Vérification :

```bash
docker compose -f docker-compose.yml -f docker-compose.override.yaml -f docker-compose.monitoring.yaml ps
curl http://localhost:8080/api/healthz
curl http://localhost:8080/api/bot/status
```

Grafana : `http://localhost:3001` (`admin` / `GRAFANA_ADMIN_PASSWORD`) → dossier **MazWorld**. Uptime Kuma : `http://localhost:3002`, configuration manuelle au premier lancement (pas d'API de provisioning stable en v1.x) :

1. Créer le compte administrateur.
2. **Settings → Notifications** → notification *Discord Webhook*, URL du salon dédié.
3. **Add New Monitor**, un par cible (`GET /`, `GET /api/healthz`, `GET /api/bot/status`), type *HTTP(s)*, intervalle 60 s, notification Discord cochée.
...
Si le rollback implique de revenir en arrière sur une migration de schéma non rétro-compatible, restaurer au préalable une sauvegarde du volume `mysql_data`, car les migrations Doctrine appliquées automatiquement au démarrage ne redescendent pas de version seules.
...
- **`release.yml`** s'exécute à la fermeture d'une pull request `release/x.y.z` → `main` : il crée automatiquement le tag Git correspondant (version lue depuis `web/frontend/package.json`) et publie une GitHub Release.
